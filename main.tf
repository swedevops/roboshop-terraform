...
module "rds" {
  source       = "git::https://github.com/swedevops/tf-module-rds.git"
  for_each     = var.rds
  subnets   =    lookup(lookup(lookup(lookup(module.vpc, "main", null), "subnets", null), each.value["subnet_name"], null), "subnet_ids", null)
  allow_db_cidr =  lookup(lookup(lookup(lookup(module.vpc, "main", null), "subnets", null), each.value["allow_db_cidr"], null), "subnet_cidrs", null)
  engine_version = each.value["engine_version"]
...
module "rabbitmq" {
  source = "git::https://github.com/swedevops/tf-module-amazon-mq.git"
...
#module "alb" {
#  source = "git::https://github.com/swedevops/tf-module-alb.git"
#
#  for_each       = var.alb
#  subnets        = lookup(lookup(lookup(lookup(module.vpc, "main", null), "subnets", null), each.value["subnet_name"], null), "subnet_ids", null)
...
module "eks" {
  source             = "git::https://github.com/swedevops/tf-module-eks.git"
  ENV                = var.env
  eks_version        = 1.27
...
resource "aws_instance" "load" {
  ...
  vpc_security_group_ids = ["sg-0fc2e28757689f3e0"]
  tags = {
    Name = "load-runner"
  }
}
